class Level_3 {
    ...
    constructor(w) {
        ...
        // Keeps track of plumes of smoke
        this.smoke = [];
        // Spawns health potions for boss level
        this.potions = [];
        this.potions.push(new PowerUp(1300, 280, 1));
        this.potions.push(new PowerUp(1300, 340, 2));
        this.respawn_timer = 900; // Respawn timer for potions
    }
    /**
    ...
    initializeEnemies() {
        this.enemies = [];
        this.enemies.push(new Wizard(animation_wizard, 600, 220, 100, 100, 4));
        this.enemies.push(new Wizard(animation_wizard, 600, 270, 100, 100, 4));
        this.enemies.push(new Boss(animation_boss, animation_boss_attack, 1200, 220, 150, 150, 50));
    }
    /**
    ...
    draw() {
        ...
        for (let i = 0; i < this.potions.length; ++i) {
            this.potions[i].draw();
            if (this.respawn_timer == 0 && i == 0) {
                this.potions[i].x = 1300;
                this.potions[i].y = 280;
                this.potions[i].offset = 0;
                this.potions[i+1].x = 1300;
                this.potions[i+1].y = 340;
                this.potions[i+1].offset = 0;
            }
        }
